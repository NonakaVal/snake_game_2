from tkinter import *
import random

# Game 'Settings' (const)
GAME_WIDTH = 500
GAME_HEIGHT = 500
SPEED = 80
SPACE_SIZE = 20
BODY_PARTS = 4
SNAKE_COLOR = '#F46C74'
FOOD_COLOR = '#F46C74'
BG_COLOR = '#353E49'


class Snake:
    def __init__(self):
        """Creates Snake object
        """
        self.body_size = BODY_PARTS
        self.coordinates = []
        self.squares = []

        for _ in range(BODY_PARTS):
            self.coordinates.append([0, 0])

        for x, y in self.coordinates:
            square = canvas.create_rectangle(x, y, x + SPACE_SIZE, y + SPACE_SIZE, fill=SNAKE_COLOR, tag="snake")
            self.squares.append(square)


class Food:
    def __init__(self):
        """Create Snake's food
        """
        x = random.randint(0, (GAME_WIDTH // SPACE_SIZE) - 1) * SPACE_SIZE
        y = random.randint(0, (GAME_HEIGHT // SPACE_SIZE) - 1) * SPACE_SIZE

        self.coordinates = [x, y]
        canvas.create_oval(x, y, x + SPACE_SIZE, y + SPACE_SIZE, fill=FOOD_COLOR, tag="food")


def next_turn(snake, food):
    """Defines Game interactions
    """
    x, y = snake.coordinates[0]

    direction_mapping = {
        "up": (0, -SPACE_SIZE),
        "down": (0, SPACE_SIZE),
        "left": (-SPACE_SIZE, 0),
        "right": (SPACE_SIZE, 0)
    }

    x_adjust, y_adjust = direction_mapping.get(direction, (0, 0))

    x += x_adjust
    y += y_adjust

    snake.coordinates.insert(0, (x, y))
    square = canvas.create_rectangle(x, y, x + SPACE_SIZE, y + SPACE_SIZE, fill=SNAKE_COLOR)
    snake.squares.insert(0, square)

    if x == food.coordinates[0] and y == food.coordinates[1]:
        global score
        score += 1
        label.config(text=f"Score: {score}")
        canvas.delete('food')
        food = Food()
    else:
        del snake.coordinates[-1]
        canvas.delete(snake.squares[-1])
        del snake.squares[-1]

    if check_collisions(snake):
        game_over()
    else:
        window.after(SPEED, next_turn, snake, food)


def change_direction(new_direction):
    """Snake control
    """
    global direction
    opposite_directions = {'left': 'right', 'right': 'left', 'up': 'down', 'down': 'up'}

    if new_direction != opposite_directions.get(direction):
        direction = new_direction


def check_collisions(snake):
    """Define limits for collision
    """
    x, y = snake.coordinates[0]
    if x < 0 or x >= GAME_WIDTH or y < 0 or x >= GAME_HEIGHT:
        return True
    for body_part in snake.coordinates[1:]:
        if x == body_part[0] and y == body_part[1]:
            return True
    return False


def game_over():
    """Create Game over message
    """
    canvas.delete(ALL)
    canvas.create_text(canvas.winfo_width() / 2, canvas.winfo_height() / 2, font=('consolas', 50),
                       text='GAME OVER', fill='red', tag='gameover')


def initialize_window():
    """Graphic settings
    """
    window = Tk()
    window.title("Snake Game")
    window.resizable(False, False)

    label = Label(window, text="Score: 0", font=('console', 20))
    label.pack()

    canvas = Canvas(window, bg=BG_COLOR, height=GAME_HEIGHT, width=GAME_WIDTH)
    canvas.pack()

    return window, label, canvas


def center_window(window):
    """Center Window
    """
    window.update()
    window_width = window.winfo_width()
    window_height = window.winfo_height()
    screen_width = window.winfo_screenwidth()
    screen_height = window.winfo_screenheight()

    x = int((screen_width / 2 - (window_width / 2)))
    y = int((screen_height / 2) - (window_height / 2))

    window.geometry(f"{window_width}x{window_height}+{x}+{y}")


def setup_key_bindings(window):
    """Set Control keys
    """
    window.bind('<Left>', lambda event: change_direction('left'))
    window.bind('<Right>', lambda event: change_direction('right'))
    window.bind('<Up>', lambda event: change_direction('up'))
    window.bind('<Down>', lambda event: change_direction('down'))


def main():
    """Start the game
    """
    global window, label, canvas, score, direction
    window, label, canvas = initialize_window()
    center_window(window)
    setup_key_bindings(window)

    score = 0
    direction = 'down'

    snake = Snake()
    food = Food()

    next_turn(snake, food)

    window.mainloop()


if __name__ == "__main__":
    main()
